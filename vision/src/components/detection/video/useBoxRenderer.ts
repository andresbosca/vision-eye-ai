...
const detectFrame = async (model, videoRef, canvasRef, labels, dimensions) => {
  try {
    const batched = tf.tidy(() => {
      ...
      const small = tf.image.resizeBilinear(img, [
        dimensions.height,
        dimensions.width,
      ]);
...
interface UseBoxRendererProps {
  model: tf.GraphModel;
  videoRef: MutableRefObject<undefined>;
  canvasRef: MutableRefObject<undefined>;
  shouldRender: boolean;
  labels: string[];
}
const useBoxRenderer = ({
  model,
  videoRef,
  canvasRef,
  shouldRender,
